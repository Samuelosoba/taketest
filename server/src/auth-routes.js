...
authRouter.post("/login", async (req, res) => {
  ...
  if (!u.active || (u.institution && u.institution.status !== "ACTIVE"))
    fail(403, "Your account or institution is inactive.");
  await audit(u, "SIGNED_IN", u.id);
  res.json(await session(u, res));
});
authRouter.post("/register", async (req, res) => {
    ...
    if (data.institutionName)
      institution = await tx.institution.create({
        data: {
          ...
          subscriptions: {
            create: { endsAt: new Date(Date.now() + 30 * 86400000) },
          },
        },
      });
    else
      institution = await tx.institution.findUnique({
        where: { slug: data.slug },
      });
    if (!institution || institution.status !== "ACTIVE")
      fail(400, "Institution is unavailable.");
    return tx.user.create({
      data: {
        name: data.name,
        email: data.email,
        password,
        role: data.institutionName ? "INSTITUTION_ADMIN" : "STUDENT",
  ...
  const sent = await mail(
    u.email,
    "Verify your AssessmentDesk account",
...
authRouter.post("/refresh", async (req, res) => {
  ...
  if (
    ...
    !token.user.active ||
    (token.user.institution && token.user.institution.status !== "ACTIVE")
  )
    fail(401, "Please sign in again.");
  const consumed = await db.refreshToken.deleteMany({ where: { id } });
  if (!consumed.count) fail(401, "Please sign in again.");
  res.json(await session(token.user, res));
});
authRouter.post("/logout", async (req, res) => {
  await db.refreshToken.deleteMany({
    where: { id: hash(req.cookies.refresh || "") },
  });
...
authRouter.post("/reset-password", async (req, res) => {
  const data = parse(
    z.object({ token: z.string(), password: z.string().min(10).max(128) }),
    req.body,
  );
  const password = await bcrypt.hash(data.password, 12);
  await db.$transaction(async (tx) => {
    const t = await tx.accountToken.findFirst({
      where: {
        id: hash(data.token),
        type: "RESET",
    ...
    if (!t) fail(400, "This link is invalid or expired.");
    await tx.accountToken.delete({ where: { id: t.id } });
    await tx.user.update({ where: { id: t.userId }, data: { password } });
    await tx.refreshToken.deleteMany({ where: { userId: t.userId } });
  });
  res.json({ ok: true });
});
authRouter.post("/verify-email", async (req, res) => {
  ...
  await db.$transaction(async (tx) => {
    const t = await tx.accountToken.findFirst({
      where: { id: hash(token), type: "VERIFY", expiresAt: { gt: new Date() } },
    });
    if (!t) fail(400, "This link is invalid or expired.");
    await tx.accountToken.delete({ where: { id: t.id } });
...
authRouter.post("/change-password", auth, async (req, res) => {
  ...
  if (!(await bcrypt.compare(data.currentPassword, req.user.password)))
