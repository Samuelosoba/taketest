...
import { monitoringToken } from "./proctoring.js";
...
export function markQuestions(questions, answers, previous = []) {
  return questions.map((q) => {
    ...
    if (q.type === "ESSAY")
      return (
        previous.find((item) => item.questionId === q.id) || {
          questionId: q.id,
          marks: q.marks,
          awarded: null,
          correct: false,
    ...
    const correct =
      q.type === "SHORT"
        ? !!normalize(answer) &&
          q.acceptedAnswers.some((a) => normalize(a) === normalize(answer))
        : selected.length > 0 &&
          JSON.stringify(selected) === JSON.stringify(correctIds);
    return {
      questionId: q.id,
      marks: q.marks,
  ...
  return {
    ...
    grade: pending
      ...
      : [...bands]
          ...
          .find((b) => percentage >= b.min)?.grade || "F",
    passed: !pending && percentage >= passMark,
  ...
  return db.$transaction(async (tx) => {
    const a = await tx.examAttempt.findFirst({
      where: {
        id,
        ...(user
          ? { institutionId: user.institutionId, studentId: user.id }
          : {}),
      },
      include: { result: true, exam: { include: { institution: true } } },
    });
    if (!a) fail(404, "Attempt not found.");
    if (a.status !== "STARTED") return a.result;
    const lock = await tx.examAttempt.updateMany({
      where: { id, status: "STARTED", version: a.version },
      data: {
        status: "SUBMITTED",
        version: { increment: 1 },
    ...
    const result = await tx.result.create({
      data: {
        ...
        attemptId: id,
        ...summary,
        released: a.exam.releaseResults && summary.status === "GRADED",
      },
    });
...
export async function expireAttempts() {
  const rows = await db.examAttempt.findMany({
    where: { status: "STARTED", expectedEndAt: { lte: new Date() } },
    select: { id: true },
    take: 100,
  });
  for (const a of rows) {
    try {
      await finishAttempt(a.id, null, true);
    } catch (e) {
      if (e.status !== 409 && e.code !== "P2002")
        console.error("Expiry failed", a.id, e.message);
    }
  }
}
export function candidateAttempt(a) {
  return {
    ...
    serverTime: new Date(),
    answers: a.answers,
    fullscreen: a.snapshot.fullscreen,
    cameraRequired: !!a.snapshot.cameraRequired,
    monitoringToken: a.status === "STARTED" ? monitoringToken(a) : undefined,
    questions: a.snapshot.questions.map(({ acceptedAnswers, ...q }) => ({
      ...q,
      options: q.options.map(({ correct, ...o }) => o),
    })),
  };
}
